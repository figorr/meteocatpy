...
from .const import (
    METEOCAT_ATTR_DATE,
    METEOCAT_ATTR_DAY,
    METEOCAT_ATTR_ELABORATED,
    METEOCAT_ATTR_FORECAST,
    METEOCAT_ATTR_ID,
    METEOCAT_ATTR_NAME,
    METEOCAT_ATTR_TOWN_ALTITUDE,
    METEOCAT_ATTR_TOWN_LATITUDE_DECIMAL,
    METEOCAT_ATTR_TOWN_LONGITUDE_DECIMAL,
    METEOCAT_ATTR_TOWN_RESIDENTS,
    AOD_ALTITUDE,
    AOD_CONDITION,
    AOD_COORDS,
    AOD_DISTANCE,
...
class DailyForecast:
    ...
    def __init__(self, data: dict[str, Any], zoneinfo: ZoneInfo) -> None:
        ...
        self._datetime = parse_api_timestamp(data[METEOCAT_ATTR_ELABORATED])
        ...
        for day_data in data[METEOCAT_ATTR_FORECAST][METEOCAT_ATTR_DAY]:
            day = parse_api_timestamp(day_data[METEOCAT_ATTR_DATE], zoneinfo)
            if cur_day <= day.date():
                try:
...
class Town:
    ...
    def __init__(self, data: dict[str, Any]) -> None:
        """Init METEOCAT Town."""
        self.altitude = int(data[METEOCAT_ATTR_TOWN_ALTITUDE])
        self.coords = (
            float(data[METEOCAT_ATTR_TOWN_LATITUDE_DECIMAL]),
            float(data[METEOCAT_ATTR_TOWN_LONGITUDE_DECIMAL]),
        )
        self.daily: DailyForecast | None = None
        self.distance = float(data[ATTR_DISTANCE])
        self.hourly: HourlyForecast | None = None
        self.id = str(data[METEOCAT_ATTR_ID])
        self.name = str(data[METEOCAT_ATTR_NAME])
        self.residents = int(data[METEOCAT_ATTR_TOWN_RESIDENTS])
