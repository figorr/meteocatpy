"""Client for the METEOCAT REST API."""
...
from .const import (
    ...
    METEOCAT_ATTR_TOWN_LATITUDE_DECIMAL,
    METEOCAT_ATTR_TOWN_LONGITUDE_DECIMAL,
    METEOCAT_ATTR_WEATHER_STATION_LATITUDE,
    METEOCAT_ATTR_WEATHER_STATION_LONGITUDE,
    AOD_CONDITION,
    AOD_DEW_POINT,
    AOD_FEEL_TEMP,
    AOD_HUMIDITY,
...
class ConnectionOptions:
    """METEOCAT API options for connection."""
...
class METEOCAT:
    """Interacts with the METEOCAT API."""
    ...
    async def _api_call(self, cmd: str, fetch_data: bool = False) -> dict[str, Any]:
        ...
        if isinstance(resp_json, dict):
            if resp_json.get(METEOCAT_ATTR_STATE) == 404:
                raise ApiError("API data error")

        json_response = cast(dict[str, Any], resp_json)
        if fetch_data and METEOCAT_ATTR_DATA in json_response:
            data = await self.api_data(json_response[METEOCAT_ATTR_DATA])
            json_response = {
    ...
    async def api_data(self, url: str) -> Any:
        ...
        if isinstance(resp_json, dict):
            if resp_json.get(METEOCAT_ATTR_STATE, 200) == 404:
                raise ApiError("API data error")
    ...
    async def get_climatological_values_station_by_coordinates(
        ...
        for cur_station in stations[ATTR_DATA]:
            station_coords = parse_station_coordinates(
                cur_station[METEOCAT_ATTR_WEATHER_STATION_LATITUDE],
                cur_station[METEOCAT_ATTR_WEATHER_STATION_LONGITUDE],
            )
    ...
    async def get_conventional_observation_station_by_coordinates(
        ...
        for cur_station in stations[ATTR_DATA]:
            cur_coords = (
                cur_station[METEOCAT_ATTR_STATION_LATITUDE],
                cur_station[METEOCAT_ATTR_STATION_LONGITUDE],
            )
            cur_distance = self.calc_distance(search_coords, cur_coords).km
            if cur_distance < distance:
                distance = cur_distance
    ...
    async def get_town_by_coordinates(
        ...
        for cur_town in towns[ATTR_DATA]:
            cur_coords = (
                cur_town[METEOCAT_ATTR_TOWN_LATITUDE_DECIMAL],
                cur_town[METEOCAT_ATTR_TOWN_LONGITUDE_DECIMAL],
            )
            cur_distance = self.calc_distance(search_coords, cur_coords).km
            if cur_distance < distance:
                distance = cur_distance
    ...
    async def update_daily(self) -> None:
        """Update METEOCAT town daily forecast."""
        if self.town is not None:
    ...
    async def update_hourly(self) -> None:
        """Update METEOCAT town hourly forecast."""
        if self.town is not None:
            town_id = self.town.get_id()
