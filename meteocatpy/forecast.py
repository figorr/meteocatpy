...
from .const import (
    METEOCAT_ATTR_DIRECTION,
    METEOCAT_ATTR_FEEL_TEMPERATURE,
    METEOCAT_ATTR_HUMIDITY,
    METEOCAT_ATTR_MAX,
    METEOCAT_ATTR_MIN,
    METEOCAT_ATTR_PERIOD,
    METEOCAT_ATTR_PRECIPITATION,
    METEOCAT_ATTR_PRECIPITATION_PROBABILITY,
    METEOCAT_ATTR_SKY_STATE,
    METEOCAT_ATTR_SNOW,
    METEOCAT_ATTR_SNOW_PROBABILITY,
    METEOCAT_ATTR_SPEED,
    METEOCAT_ATTR_STORM_PROBABILITY,
    METEOCAT_ATTR_SUN_RISE,
    METEOCAT_ATTR_SUN_SET,
    METEOCAT_ATTR_TEMPERATURE,
    METEOCAT_ATTR_UV_MAX,
    METEOCAT_ATTR_VALUE,
    METEOCAT_ATTR_WIND,
    METEOCAT_ATTR_WIND_GUST,
...
class DailyForecastValue(ForecastValue):
    """METEOCAT Town Daily Forecast value."""
    ...
    def __init__(self, data: dict[str, Any], dt: datetime) -> None:
        """Init METEOCAT Town Daily Forecast."""
        condition = self.parse_value(data[METEOCAT_ATTR_SKY_STATE])
        ...
        self.feel_temp_max = int(
            self.parse_value(data[METEOCAT_ATTR_FEEL_TEMPERATURE], key=METEOCAT_ATTR_MAX)
        )
        self.feel_temp_min = int(
            self.parse_value(data[METEOCAT_ATTR_FEEL_TEMPERATURE], key=METEOCAT_ATTR_MIN)
        )
        self.humidity_max = int(
            self.parse_value(data[METEOCAT_ATTR_HUMIDITY], key=METEOCAT_ATTR_MAX)
        )
        self.humidity_min = int(
            self.parse_value(data[METEOCAT_ATTR_HUMIDITY], key=METEOCAT_ATTR_MIN)
        )
        self.precipitation_prob = int(
            self.parse_value(data[METEOCAT_ATTR_PRECIPITATION_PROBABILITY])
        )
        self.temp_max = int(
            self.parse_value(data[METEOCAT_ATTR_TEMPERATURE], key=METEOCAT_ATTR_MAX)
        )
        self.temp_min = int(
            self.parse_value(data[METEOCAT_ATTR_TEMPERATURE], key=METEOCAT_ATTR_MIN)
        )
        self.wind_direction = self.parse_wind_direction(
            self.parse_value(data[METEOCAT_ATTR_WIND], key=METEOCAT_ATTR_DIRECTION),
        )

        if self.wind_direction is not None:
            self.wind_speed = int(
                self.parse_value(data[METEOCAT_ATTR_WIND], key=METEOCAT_ATTR_SPEED)
            )
        else:
            self.wind_speed = None

        if METEOCAT_ATTR_UV_MAX in data:
            self.uv_index = int(data[METEOCAT_ATTR_UV_MAX])
        else:
            self.uv_index = None
    ...
    def parse_value(
        self, values: dict[str, Any] | list[Any], key: str = METEOCAT_ATTR_VALUE
    ) -> Any:
            ...
            if len(values) > 1:
                for value in values:
                    ...
                    for period in self.periods:
                        if value[METEOCAT_ATTR_PERIOD] == period:
                            return value[key]
            else:
                if key in values[0]:
                    return values[0][key]
...
class HourlyForecastValue(ForecastValue):
    """METEOCAT Town Hourly Forecast value."""
    ...
    def __init__(self, data: dict[str, Any], dt: datetime, hour: int) -> None:
        """Init METEOCAT Town Hourly Forecast."""
        condition = self.parse_value(data[METEOCAT_ATTR_SKY_STATE], hour)
        if condition is None:
            raise ValueError(f"HourlyForecastValue {dt} {hour}:00")

        self.condition = self.parse_condition(condition)
        self._datetime = dt.replace(hour=hour)
        self.sunrise = str(data[METEOCAT_ATTR_SUN_RISE])
        self.sunset = str(data[METEOCAT_ATTR_SUN_SET])

        feel_temp = self.parse_value(data[METEOCAT_ATTR_FEEL_TEMPERATURE], hour)
        if feel_temp is not None:
            self.feel_temp = int(feel_temp)
        else:
            self.feel_temp = None

        humidity = self.parse_value(data[METEOCAT_ATTR_HUMIDITY], hour)
        if humidity is not None:
            self.humidity = int(humidity)
        else:
            self.humidity = None

        temp = self.parse_value(data[METEOCAT_ATTR_TEMPERATURE], hour)
        if temp is not None:
            self.temp = int(temp)
        else:
            self.temp = None
        ...
        rain_probability = self.parse_interval_value(
            data[METEOCAT_ATTR_PRECIPITATION_PROBABILITY], hour
        )
        if rain_probability is not None:
            self.rain_probability = int(rain_probability)

            rain = self.parse_value(data[METEOCAT_ATTR_PRECIPITATION], hour)
            if rain is not None:
        ...
        snow_probability = self.parse_interval_value(
            data[METEOCAT_ATTR_SNOW_PROBABILITY], hour
        )
        if snow_probability is not None:
            self.snow_probability = int(snow_probability)

            snow = self.parse_value(data[METEOCAT_ATTR_SNOW], hour)
            if snow is not None:
        ...
        storm_probability = self.parse_interval_value(
            data[METEOCAT_ATTR_STORM_PROBABILITY], hour
        )
        ...
        wind_direction = self.parse_value(
            data[METEOCAT_ATTR_WIND_GUST],
            hour,
            key=METEOCAT_ATTR_DIRECTION,
        )
        ...
        if self.wind_direction is not None:
            self.wind_speed = float(
                self.parse_value(
                    data[METEOCAT_ATTR_WIND_GUST], hour, key=METEOCAT_ATTR_SPEED
                )[0]
            )
            self.wind_speed_max = float(
                self.parse_value(data[METEOCAT_ATTR_WIND_GUST], hour)
            )
        else:
            self.wind_speed = None
    ...
    def parse_interval_value(
        self, values: Any, hour: int, key: str = METEOCAT_ATTR_VALUE
    ) -> Any:
        ...
        for value in values:
            if key not in value:
                continue
            period = value[METEOCAT_ATTR_PERIOD]
    ...
    def parse_value(self, values: Any, hour: int, key: str = METEOCAT_ATTR_VALUE) -> Any:
        """Parse Town hourly forecast value from data."""
        for value in values:
            if key not in value:
                continue
            if int(value[METEOCAT_ATTR_PERIOD]) == hour:
                return None if not value[key] else value[key]
        return None