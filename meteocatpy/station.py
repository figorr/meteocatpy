...
from .const import (
    METEOCAT_ATTR_IDEMA,
    METEOCAT_ATTR_STATION_ALTITUDE,
    METEOCAT_ATTR_STATION_DATE,
    METEOCAT_ATTR_STATION_DEWPOINT,
    METEOCAT_ATTR_STATION_HUMIDITY,
    METEOCAT_ATTR_STATION_LATITUDE,
    METEOCAT_ATTR_STATION_LOCATION,
    METEOCAT_ATTR_STATION_LONGITUDE,
    METEOCAT_ATTR_STATION_PRECIPITATION,
    METEOCAT_ATTR_STATION_PRESSURE,
    METEOCAT_ATTR_STATION_PRESSURE_SEA,
    METEOCAT_ATTR_STATION_TEMPERATURE,
    METEOCAT_ATTR_STATION_TEMPERATURE_MAX,
    METEOCAT_ATTR_STATION_TEMPERATURE_MIN,
    METEOCAT_ATTR_STATION_WIND_DIRECTION,
    METEOCAT_ATTR_STATION_WIND_SPEED,
    METEOCAT_ATTR_STATION_WIND_SPEED_MAX,
    AOD_ALTITUDE,
    AOD_COORDS,
    AOD_DATETIME,
    AOD_DEW_POINT,
...
class Station:
    ...
    def __init__(self, data: dict[str, Any]) -> None:
        """Init METEOCAT Station."""

        self.altitude = float(data[METEOCAT_ATTR_STATION_ALTITUDE])
        self.coords = (
            float(data[METEOCAT_ATTR_STATION_LATITUDE]),
            float(data[METEOCAT_ATTR_STATION_LONGITUDE]),
        )
        self.distance = float(data[ATTR_DISTANCE])
        self.id = str(data[METEOCAT_ATTR_IDEMA])
        self.name = str(data[METEOCAT_ATTR_STATION_LOCATION])
    ...
    def update_sample(self, data: dict[str, Any]) -> None:
        ...
        station_dt = parse_api_timestamp(data[METEOCAT_ATTR_STATION_DATE])

        self._datetime = station_dt.astimezone(self.get_timezone())

        if METEOCAT_ATTR_STATION_DEWPOINT in data:
            self.dew_point = float(data[METEOCAT_ATTR_STATION_DEWPOINT])

        if METEOCAT_ATTR_STATION_HUMIDITY in data:
            self.humidity = float(data[METEOCAT_ATTR_STATION_HUMIDITY])

        if METEOCAT_ATTR_STATION_PRECIPITATION in data:
            self.precipitation = float(data[METEOCAT_ATTR_STATION_PRECIPITATION])

        if METEOCAT_ATTR_STATION_PRESSURE_SEA in data:
            self.pressure = float(data[METEOCAT_ATTR_STATION_PRESSURE_SEA])
        elif METEOCAT_ATTR_STATION_PRESSURE in data:
            self.pressure = float(data[METEOCAT_ATTR_STATION_PRESSURE])

        if METEOCAT_ATTR_STATION_TEMPERATURE in data:
            self.temp = float(data[METEOCAT_ATTR_STATION_TEMPERATURE])

        if METEOCAT_ATTR_STATION_TEMPERATURE_MAX in data:
            self.temp_max = float(data[METEOCAT_ATTR_STATION_TEMPERATURE_MAX])

        if METEOCAT_ATTR_STATION_TEMPERATURE_MIN in data:
            self.temp_min = float(data[METEOCAT_ATTR_STATION_TEMPERATURE_MIN])

        if METEOCAT_ATTR_STATION_WIND_DIRECTION in data:
            self.wind_direction = float(data[METEOCAT_ATTR_STATION_WIND_DIRECTION])

        if METEOCAT_ATTR_STATION_WIND_SPEED in data:
            self.wind_speed = float(data[METEOCAT_ATTR_STATION_WIND_SPEED])

        if METEOCAT_ATTR_STATION_WIND_SPEED_MAX in data:
            self.wind_speed_max = float(data[METEOCAT_ATTR_STATION_WIND_SPEED_MAX])
    ...
    def update_samples(self, samples: dict[str, Any]) -> None:
        ...
        for sample in samples[ATTR_DATA]:
            dt = parse_api_timestamp(sample[METEOCAT_ATTR_STATION_DATE])
