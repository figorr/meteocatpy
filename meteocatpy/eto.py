...
from .const import BASE_URL, ETO_URL
...
class MeteocatETO:
    """Clase para interactuar con los datos de evapotranspiración de la API de Meteocat."""

    def __init__(self, api_key: str):
        """
        Inicializa la clase MeteocatETO.
    ...
    async def get_eto_data(self, station_id: str):
        """
        Obtiene los datos de evapotranspiración desde la API de Meteocat.

        Args:
            station_id (str): Código de la estación.

        Returns:
            dict: Datos de evapotranspiración de la estación.
        """
        any, mes, dia = self.get_current_date()  # Calcula la fecha actual
        url = f"{BASE_URL}{ETO_URL}".format(
                    ...
                    if response.status == 400:
                        ...
                        if "message" in error_data:
                            ...
                            if match:
                                ...
                                new_url = f"{BASE_URL}{ETO_URL}".format(
                                    codiEstacio=station_id, any=any, mes=f"{mes:02d}", dia=f"{dia:02d}"
                                )
